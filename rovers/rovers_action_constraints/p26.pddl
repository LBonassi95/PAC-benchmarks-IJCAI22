(define (problem roverprob3965) (:domain Rover)
(:objects
	general - Lander
	colour high_res low_res - Mode
	rover0 rover1 rover2 rover3 rover4 rover5 rover6 rover7 rover8 rover9 - Rover
	rover0store rover1store rover2store rover3store rover4store rover5store rover6store rover7store rover8store rover9store - Store
	waypoint0 waypoint1 waypoint2 waypoint3 waypoint4 waypoint5 waypoint6 waypoint7 waypoint8 waypoint9 waypoint10 waypoint11 waypoint12 waypoint13 waypoint14 waypoint15 waypoint16 waypoint17 waypoint18 waypoint19 waypoint20 waypoint21 waypoint22 waypoint23 waypoint24 waypoint25 waypoint26 waypoint27 waypoint28 waypoint29 - Waypoint
	camera0 camera1 camera2 camera3 camera4 camera5 camera6 - Camera
	objective0 objective1 objective2 objective3 objective4 objective5 - Objective
	)
(:init
	(visible waypoint0 waypoint11)
	(visible waypoint11 waypoint0)
	(visible waypoint0 waypoint29)
	(visible waypoint29 waypoint0)
	(visible waypoint1 waypoint0)
	(visible waypoint0 waypoint1)
	(visible waypoint1 waypoint4)
	(visible waypoint4 waypoint1)
	(visible waypoint1 waypoint6)
	(visible waypoint6 waypoint1)
	(visible waypoint1 waypoint15)
	(visible waypoint15 waypoint1)
	(visible waypoint1 waypoint25)
	(visible waypoint25 waypoint1)
	(visible waypoint2 waypoint9)
	(visible waypoint9 waypoint2)
	(visible waypoint2 waypoint16)
	(visible waypoint16 waypoint2)
	(visible waypoint2 waypoint24)
	(visible waypoint24 waypoint2)
	(visible waypoint3 waypoint10)
	(visible waypoint10 waypoint3)
	(visible waypoint3 waypoint13)
	(visible waypoint13 waypoint3)
	(visible waypoint3 waypoint26)
	(visible waypoint26 waypoint3)
	(visible waypoint3 waypoint27)
	(visible waypoint27 waypoint3)
	(visible waypoint4 waypoint6)
	(visible waypoint6 waypoint4)
	(visible waypoint4 waypoint9)
	(visible waypoint9 waypoint4)
	(visible waypoint4 waypoint10)
	(visible waypoint10 waypoint4)
	(visible waypoint5 waypoint1)
	(visible waypoint1 waypoint5)
	(visible waypoint5 waypoint6)
	(visible waypoint6 waypoint5)
	(visible waypoint5 waypoint11)
	(visible waypoint11 waypoint5)
	(visible waypoint5 waypoint14)
	(visible waypoint14 waypoint5)
	(visible waypoint5 waypoint19)
	(visible waypoint19 waypoint5)
	(visible waypoint5 waypoint23)
	(visible waypoint23 waypoint5)
	(visible waypoint6 waypoint11)
	(visible waypoint11 waypoint6)
	(visible waypoint6 waypoint19)
	(visible waypoint19 waypoint6)
	(visible waypoint6 waypoint22)
	(visible waypoint22 waypoint6)
	(visible waypoint7 waypoint5)
	(visible waypoint5 waypoint7)
	(visible waypoint7 waypoint6)
	(visible waypoint6 waypoint7)
	(visible waypoint7 waypoint14)
	(visible waypoint14 waypoint7)
	(visible waypoint7 waypoint20)
	(visible waypoint20 waypoint7)
	(visible waypoint7 waypoint21)
	(visible waypoint21 waypoint7)
	(visible waypoint7 waypoint23)
	(visible waypoint23 waypoint7)
	(visible waypoint7 waypoint27)
	(visible waypoint27 waypoint7)
	(visible waypoint8 waypoint0)
	(visible waypoint0 waypoint8)
	(visible waypoint8 waypoint2)
	(visible waypoint2 waypoint8)
	(visible waypoint8 waypoint14)
	(visible waypoint14 waypoint8)
	(visible waypoint8 waypoint15)
	(visible waypoint15 waypoint8)
	(visible waypoint8 waypoint25)
	(visible waypoint25 waypoint8)
	(visible waypoint8 waypoint27)
	(visible waypoint27 waypoint8)
	(visible waypoint9 waypoint10)
	(visible waypoint10 waypoint9)
	(visible waypoint9 waypoint24)
	(visible waypoint24 waypoint9)
	(visible waypoint10 waypoint8)
	(visible waypoint8 waypoint10)
	(visible waypoint10 waypoint15)
	(visible waypoint15 waypoint10)
	(visible waypoint10 waypoint20)
	(visible waypoint20 waypoint10)
	(visible waypoint10 waypoint21)
	(visible waypoint21 waypoint10)
	(visible waypoint10 waypoint23)
	(visible waypoint23 waypoint10)
	(visible waypoint11 waypoint16)
	(visible waypoint16 waypoint11)
	(visible waypoint11 waypoint18)
	(visible waypoint18 waypoint11)
	(visible waypoint12 waypoint1)
	(visible waypoint1 waypoint12)
	(visible waypoint12 waypoint4)
	(visible waypoint4 waypoint12)
	(visible waypoint12 waypoint6)
	(visible waypoint6 waypoint12)
	(visible waypoint12 waypoint9)
	(visible waypoint9 waypoint12)
	(visible waypoint12 waypoint16)
	(visible waypoint16 waypoint12)
	(visible waypoint12 waypoint23)
	(visible waypoint23 waypoint12)
	(visible waypoint13 waypoint5)
	(visible waypoint5 waypoint13)
	(visible waypoint13 waypoint20)
	(visible waypoint20 waypoint13)
	(visible waypoint14 waypoint6)
	(visible waypoint6 waypoint14)
	(visible waypoint14 waypoint10)
	(visible waypoint10 waypoint14)
	(visible waypoint14 waypoint21)
	(visible waypoint21 waypoint14)
	(visible waypoint15 waypoint22)
	(visible waypoint22 waypoint15)
	(visible waypoint15 waypoint23)
	(visible waypoint23 waypoint15)
	(visible waypoint15 waypoint27)
	(visible waypoint27 waypoint15)
	(visible waypoint15 waypoint28)
	(visible waypoint28 waypoint15)
	(visible waypoint16 waypoint9)
	(visible waypoint9 waypoint16)
	(visible waypoint16 waypoint14)
	(visible waypoint14 waypoint16)
	(visible waypoint17 waypoint10)
	(visible waypoint10 waypoint17)
	(visible waypoint17 waypoint24)
	(visible waypoint24 waypoint17)
	(visible waypoint17 waypoint25)
	(visible waypoint25 waypoint17)
	(visible waypoint18 waypoint10)
	(visible waypoint10 waypoint18)
	(visible waypoint18 waypoint14)
	(visible waypoint14 waypoint18)
	(visible waypoint18 waypoint16)
	(visible waypoint16 waypoint18)
	(visible waypoint18 waypoint27)
	(visible waypoint27 waypoint18)
	(visible waypoint19 waypoint7)
	(visible waypoint7 waypoint19)
	(visible waypoint19 waypoint10)
	(visible waypoint10 waypoint19)
	(visible waypoint19 waypoint12)
	(visible waypoint12 waypoint19)
	(visible waypoint19 waypoint15)
	(visible waypoint15 waypoint19)
	(visible waypoint19 waypoint21)
	(visible waypoint21 waypoint19)
	(visible waypoint20 waypoint8)
	(visible waypoint8 waypoint20)
	(visible waypoint20 waypoint11)
	(visible waypoint11 waypoint20)
	(visible waypoint20 waypoint15)
	(visible waypoint15 waypoint20)
	(visible waypoint20 waypoint16)
	(visible waypoint16 waypoint20)
	(visible waypoint20 waypoint21)
	(visible waypoint21 waypoint20)
	(visible waypoint20 waypoint28)
	(visible waypoint28 waypoint20)
	(visible waypoint21 waypoint2)
	(visible waypoint2 waypoint21)
	(visible waypoint21 waypoint3)
	(visible waypoint3 waypoint21)
	(visible waypoint21 waypoint8)
	(visible waypoint8 waypoint21)
	(visible waypoint21 waypoint13)
	(visible waypoint13 waypoint21)
	(visible waypoint21 waypoint22)
	(visible waypoint22 waypoint21)
	(visible waypoint21 waypoint23)
	(visible waypoint23 waypoint21)
	(visible waypoint21 waypoint26)
	(visible waypoint26 waypoint21)
	(visible waypoint22 waypoint18)
	(visible waypoint18 waypoint22)
	(visible waypoint22 waypoint28)
	(visible waypoint28 waypoint22)
	(visible waypoint22 waypoint29)
	(visible waypoint29 waypoint22)
	(visible waypoint23 waypoint1)
	(visible waypoint1 waypoint23)
	(visible waypoint23 waypoint17)
	(visible waypoint17 waypoint23)
	(visible waypoint23 waypoint28)
	(visible waypoint28 waypoint23)
	(visible waypoint24 waypoint8)
	(visible waypoint8 waypoint24)
	(visible waypoint24 waypoint10)
	(visible waypoint10 waypoint24)
	(visible waypoint24 waypoint13)
	(visible waypoint13 waypoint24)
	(visible waypoint24 waypoint15)
	(visible waypoint15 waypoint24)
	(visible waypoint24 waypoint21)
	(visible waypoint21 waypoint24)
	(visible waypoint24 waypoint22)
	(visible waypoint22 waypoint24)
	(visible waypoint25 waypoint2)
	(visible waypoint2 waypoint25)
	(visible waypoint25 waypoint14)
	(visible waypoint14 waypoint25)
	(visible waypoint26 waypoint9)
	(visible waypoint9 waypoint26)
	(visible waypoint26 waypoint10)
	(visible waypoint10 waypoint26)
	(visible waypoint26 waypoint22)
	(visible waypoint22 waypoint26)
	(visible waypoint27 waypoint22)
	(visible waypoint22 waypoint27)
	(visible waypoint27 waypoint24)
	(visible waypoint24 waypoint27)
	(visible waypoint27 waypoint28)
	(visible waypoint28 waypoint27)
	(visible waypoint28 waypoint6)
	(visible waypoint6 waypoint28)
	(visible waypoint28 waypoint17)
	(visible waypoint17 waypoint28)
	(visible waypoint28 waypoint24)
	(visible waypoint24 waypoint28)
	(visible waypoint28 waypoint25)
	(visible waypoint25 waypoint28)
	(visible waypoint28 waypoint29)
	(visible waypoint29 waypoint28)
	(visible waypoint29 waypoint7)
	(visible waypoint7 waypoint29)
	(visible waypoint29 waypoint10)
	(visible waypoint10 waypoint29)
	(visible waypoint29 waypoint19)
	(visible waypoint19 waypoint29)
	(at_soil_sample waypoint0)
	(at_soil_sample waypoint1)
	(at_soil_sample waypoint2)
	(at_soil_sample waypoint3)
	(at_rock_sample waypoint3)
	(at_rock_sample waypoint4)
	(at_rock_sample waypoint6)
	(at_rock_sample waypoint8)
	(at_rock_sample waypoint9)
	(at_rock_sample waypoint10)
	(at_soil_sample waypoint11)
	(at_soil_sample waypoint12)
	(at_rock_sample waypoint12)
	(at_rock_sample waypoint14)
	(at_rock_sample waypoint15)
	(at_rock_sample waypoint19)
	(at_rock_sample waypoint20)
	(at_soil_sample waypoint21)
	(at_soil_sample waypoint23)
	(at_soil_sample waypoint24)
	(at_rock_sample waypoint24)
	(at_soil_sample waypoint25)
	(at_rock_sample waypoint25)
	(at_rock_sample waypoint26)
	(at_rock_sample waypoint28)
	(at_rock_sample waypoint29)
	(at_lander general waypoint27)
	(channel_free general)
	(at rover0 waypoint14)
	(available rover0)
	(store_of rover0store rover0)
	(empty rover0store)
	(equipped_for_rock_analysis rover0)
	(equipped_for_imaging rover0)
	(can_traverse rover0 waypoint14 waypoint5)
	(can_traverse rover0 waypoint5 waypoint14)
	(can_traverse rover0 waypoint14 waypoint6)
	(can_traverse rover0 waypoint6 waypoint14)
	(can_traverse rover0 waypoint14 waypoint7)
	(can_traverse rover0 waypoint7 waypoint14)
	(can_traverse rover0 waypoint14 waypoint8)
	(can_traverse rover0 waypoint8 waypoint14)
	(can_traverse rover0 waypoint14 waypoint16)
	(can_traverse rover0 waypoint16 waypoint14)
	(can_traverse rover0 waypoint14 waypoint18)
	(can_traverse rover0 waypoint18 waypoint14)
	(can_traverse rover0 waypoint14 waypoint25)
	(can_traverse rover0 waypoint25 waypoint14)
	(can_traverse rover0 waypoint5 waypoint1)
	(can_traverse rover0 waypoint1 waypoint5)
	(can_traverse rover0 waypoint5 waypoint11)
	(can_traverse rover0 waypoint11 waypoint5)
	(can_traverse rover0 waypoint5 waypoint13)
	(can_traverse rover0 waypoint13 waypoint5)
	(can_traverse rover0 waypoint5 waypoint23)
	(can_traverse rover0 waypoint23 waypoint5)
	(can_traverse rover0 waypoint6 waypoint4)
	(can_traverse rover0 waypoint4 waypoint6)
	(can_traverse rover0 waypoint6 waypoint19)
	(can_traverse rover0 waypoint19 waypoint6)
	(can_traverse rover0 waypoint6 waypoint28)
	(can_traverse rover0 waypoint28 waypoint6)
	(can_traverse rover0 waypoint7 waypoint20)
	(can_traverse rover0 waypoint20 waypoint7)
	(can_traverse rover0 waypoint7 waypoint21)
	(can_traverse rover0 waypoint21 waypoint7)
	(can_traverse rover0 waypoint7 waypoint27)
	(can_traverse rover0 waypoint27 waypoint7)
	(can_traverse rover0 waypoint8 waypoint0)
	(can_traverse rover0 waypoint0 waypoint8)
	(can_traverse rover0 waypoint8 waypoint2)
	(can_traverse rover0 waypoint2 waypoint8)
	(can_traverse rover0 waypoint8 waypoint10)
	(can_traverse rover0 waypoint10 waypoint8)
	(can_traverse rover0 waypoint8 waypoint15)
	(can_traverse rover0 waypoint15 waypoint8)
	(can_traverse rover0 waypoint8 waypoint24)
	(can_traverse rover0 waypoint24 waypoint8)
	(can_traverse rover0 waypoint16 waypoint9)
	(can_traverse rover0 waypoint9 waypoint16)
	(can_traverse rover0 waypoint25 waypoint17)
	(can_traverse rover0 waypoint17 waypoint25)
	(can_traverse rover0 waypoint1 waypoint12)
	(can_traverse rover0 waypoint12 waypoint1)
	(can_traverse rover0 waypoint13 waypoint3)
	(can_traverse rover0 waypoint3 waypoint13)
	(can_traverse rover0 waypoint28 waypoint22)
	(can_traverse rover0 waypoint22 waypoint28)
	(can_traverse rover0 waypoint28 waypoint29)
	(can_traverse rover0 waypoint29 waypoint28)
	(can_traverse rover0 waypoint21 waypoint26)
	(can_traverse rover0 waypoint26 waypoint21)
	(at rover1 waypoint14)
	(available rover1)
	(store_of rover1store rover1)
	(empty rover1store)
	(equipped_for_imaging rover1)
	(can_traverse rover1 waypoint14 waypoint5)
	(can_traverse rover1 waypoint5 waypoint14)
	(can_traverse rover1 waypoint14 waypoint6)
	(can_traverse rover1 waypoint6 waypoint14)
	(can_traverse rover1 waypoint14 waypoint7)
	(can_traverse rover1 waypoint7 waypoint14)
	(can_traverse rover1 waypoint14 waypoint8)
	(can_traverse rover1 waypoint8 waypoint14)
	(can_traverse rover1 waypoint14 waypoint10)
	(can_traverse rover1 waypoint10 waypoint14)
	(can_traverse rover1 waypoint14 waypoint16)
	(can_traverse rover1 waypoint16 waypoint14)
	(can_traverse rover1 waypoint14 waypoint18)
	(can_traverse rover1 waypoint18 waypoint14)
	(can_traverse rover1 waypoint14 waypoint21)
	(can_traverse rover1 waypoint21 waypoint14)
	(can_traverse rover1 waypoint14 waypoint25)
	(can_traverse rover1 waypoint25 waypoint14)
	(can_traverse rover1 waypoint5 waypoint1)
	(can_traverse rover1 waypoint1 waypoint5)
	(can_traverse rover1 waypoint5 waypoint11)
	(can_traverse rover1 waypoint11 waypoint5)
	(can_traverse rover1 waypoint5 waypoint13)
	(can_traverse rover1 waypoint13 waypoint5)
	(can_traverse rover1 waypoint5 waypoint19)
	(can_traverse rover1 waypoint19 waypoint5)
	(can_traverse rover1 waypoint5 waypoint23)
	(can_traverse rover1 waypoint23 waypoint5)
	(can_traverse rover1 waypoint6 waypoint22)
	(can_traverse rover1 waypoint22 waypoint6)
	(can_traverse rover1 waypoint6 waypoint28)
	(can_traverse rover1 waypoint28 waypoint6)
	(can_traverse rover1 waypoint7 waypoint27)
	(can_traverse rover1 waypoint27 waypoint7)
	(can_traverse rover1 waypoint8 waypoint0)
	(can_traverse rover1 waypoint0 waypoint8)
	(can_traverse rover1 waypoint8 waypoint2)
	(can_traverse rover1 waypoint2 waypoint8)
	(can_traverse rover1 waypoint8 waypoint15)
	(can_traverse rover1 waypoint15 waypoint8)
	(can_traverse rover1 waypoint8 waypoint24)
	(can_traverse rover1 waypoint24 waypoint8)
	(can_traverse rover1 waypoint10 waypoint4)
	(can_traverse rover1 waypoint4 waypoint10)
	(can_traverse rover1 waypoint10 waypoint9)
	(can_traverse rover1 waypoint9 waypoint10)
	(can_traverse rover1 waypoint10 waypoint17)
	(can_traverse rover1 waypoint17 waypoint10)
	(can_traverse rover1 waypoint10 waypoint20)
	(can_traverse rover1 waypoint20 waypoint10)
	(can_traverse rover1 waypoint10 waypoint26)
	(can_traverse rover1 waypoint26 waypoint10)
	(can_traverse rover1 waypoint10 waypoint29)
	(can_traverse rover1 waypoint29 waypoint10)
	(can_traverse rover1 waypoint16 waypoint12)
	(can_traverse rover1 waypoint12 waypoint16)
	(can_traverse rover1 waypoint21 waypoint3)
	(can_traverse rover1 waypoint3 waypoint21)
	(at rover2 waypoint16)
	(available rover2)
	(store_of rover2store rover2)
	(empty rover2store)
	(equipped_for_soil_analysis rover2)
	(equipped_for_rock_analysis rover2)
	(can_traverse rover2 waypoint16 waypoint2)
	(can_traverse rover2 waypoint2 waypoint16)
	(can_traverse rover2 waypoint16 waypoint11)
	(can_traverse rover2 waypoint11 waypoint16)
	(can_traverse rover2 waypoint16 waypoint14)
	(can_traverse rover2 waypoint14 waypoint16)
	(can_traverse rover2 waypoint16 waypoint18)
	(can_traverse rover2 waypoint18 waypoint16)
	(can_traverse rover2 waypoint2 waypoint9)
	(can_traverse rover2 waypoint9 waypoint2)
	(can_traverse rover2 waypoint2 waypoint21)
	(can_traverse rover2 waypoint21 waypoint2)
	(can_traverse rover2 waypoint2 waypoint24)
	(can_traverse rover2 waypoint24 waypoint2)
	(can_traverse rover2 waypoint2 waypoint25)
	(can_traverse rover2 waypoint25 waypoint2)
	(can_traverse rover2 waypoint11 waypoint0)
	(can_traverse rover2 waypoint0 waypoint11)
	(can_traverse rover2 waypoint14 waypoint5)
	(can_traverse rover2 waypoint5 waypoint14)
	(can_traverse rover2 waypoint14 waypoint6)
	(can_traverse rover2 waypoint6 waypoint14)
	(can_traverse rover2 waypoint14 waypoint7)
	(can_traverse rover2 waypoint7 waypoint14)
	(can_traverse rover2 waypoint14 waypoint8)
	(can_traverse rover2 waypoint8 waypoint14)
	(can_traverse rover2 waypoint18 waypoint22)
	(can_traverse rover2 waypoint22 waypoint18)
	(can_traverse rover2 waypoint18 waypoint27)
	(can_traverse rover2 waypoint27 waypoint18)
	(can_traverse rover2 waypoint9 waypoint4)
	(can_traverse rover2 waypoint4 waypoint9)
	(can_traverse rover2 waypoint9 waypoint10)
	(can_traverse rover2 waypoint10 waypoint9)
	(can_traverse rover2 waypoint9 waypoint12)
	(can_traverse rover2 waypoint12 waypoint9)
	(can_traverse rover2 waypoint9 waypoint26)
	(can_traverse rover2 waypoint26 waypoint9)
	(can_traverse rover2 waypoint21 waypoint3)
	(can_traverse rover2 waypoint3 waypoint21)
	(can_traverse rover2 waypoint21 waypoint19)
	(can_traverse rover2 waypoint19 waypoint21)
	(can_traverse rover2 waypoint24 waypoint13)
	(can_traverse rover2 waypoint13 waypoint24)
	(can_traverse rover2 waypoint24 waypoint28)
	(can_traverse rover2 waypoint28 waypoint24)
	(can_traverse rover2 waypoint25 waypoint1)
	(can_traverse rover2 waypoint1 waypoint25)
	(can_traverse rover2 waypoint25 waypoint17)
	(can_traverse rover2 waypoint17 waypoint25)
	(at rover3 waypoint3)
	(available rover3)
	(store_of rover3store rover3)
	(empty rover3store)
	(equipped_for_soil_analysis rover3)
	(equipped_for_rock_analysis rover3)
	(equipped_for_imaging rover3)
	(can_traverse rover3 waypoint3 waypoint10)
	(can_traverse rover3 waypoint10 waypoint3)
	(can_traverse rover3 waypoint3 waypoint13)
	(can_traverse rover3 waypoint13 waypoint3)
	(can_traverse rover3 waypoint3 waypoint27)
	(can_traverse rover3 waypoint27 waypoint3)
	(can_traverse rover3 waypoint10 waypoint4)
	(can_traverse rover3 waypoint4 waypoint10)
	(can_traverse rover3 waypoint10 waypoint9)
	(can_traverse rover3 waypoint9 waypoint10)
	(can_traverse rover3 waypoint10 waypoint14)
	(can_traverse rover3 waypoint14 waypoint10)
	(can_traverse rover3 waypoint10 waypoint17)
	(can_traverse rover3 waypoint17 waypoint10)
	(can_traverse rover3 waypoint10 waypoint18)
	(can_traverse rover3 waypoint18 waypoint10)
	(can_traverse rover3 waypoint10 waypoint19)
	(can_traverse rover3 waypoint19 waypoint10)
	(can_traverse rover3 waypoint10 waypoint20)
	(can_traverse rover3 waypoint20 waypoint10)
	(can_traverse rover3 waypoint10 waypoint21)
	(can_traverse rover3 waypoint21 waypoint10)
	(can_traverse rover3 waypoint10 waypoint26)
	(can_traverse rover3 waypoint26 waypoint10)
	(can_traverse rover3 waypoint10 waypoint29)
	(can_traverse rover3 waypoint29 waypoint10)
	(can_traverse rover3 waypoint27 waypoint7)
	(can_traverse rover3 waypoint7 waypoint27)
	(can_traverse rover3 waypoint27 waypoint8)
	(can_traverse rover3 waypoint8 waypoint27)
	(can_traverse rover3 waypoint27 waypoint15)
	(can_traverse rover3 waypoint15 waypoint27)
	(can_traverse rover3 waypoint27 waypoint24)
	(can_traverse rover3 waypoint24 waypoint27)
	(can_traverse rover3 waypoint4 waypoint6)
	(can_traverse rover3 waypoint6 waypoint4)
	(can_traverse rover3 waypoint4 waypoint12)
	(can_traverse rover3 waypoint12 waypoint4)
	(can_traverse rover3 waypoint9 waypoint16)
	(can_traverse rover3 waypoint16 waypoint9)
	(can_traverse rover3 waypoint14 waypoint5)
	(can_traverse rover3 waypoint5 waypoint14)
	(can_traverse rover3 waypoint17 waypoint23)
	(can_traverse rover3 waypoint23 waypoint17)
	(can_traverse rover3 waypoint17 waypoint28)
	(can_traverse rover3 waypoint28 waypoint17)
	(can_traverse rover3 waypoint18 waypoint22)
	(can_traverse rover3 waypoint22 waypoint18)
	(can_traverse rover3 waypoint21 waypoint2)
	(can_traverse rover3 waypoint2 waypoint21)
	(can_traverse rover3 waypoint29 waypoint0)
	(can_traverse rover3 waypoint0 waypoint29)
	(can_traverse rover3 waypoint8 waypoint25)
	(can_traverse rover3 waypoint25 waypoint8)
	(can_traverse rover3 waypoint6 waypoint11)
	(can_traverse rover3 waypoint11 waypoint6)
	(can_traverse rover3 waypoint23 waypoint1)
	(can_traverse rover3 waypoint1 waypoint23)
	(at rover4 waypoint18)
	(available rover4)
	(store_of rover4store rover4)
	(empty rover4store)
	(equipped_for_rock_analysis rover4)
	(equipped_for_imaging rover4)
	(can_traverse rover4 waypoint18 waypoint10)
	(can_traverse rover4 waypoint10 waypoint18)
	(can_traverse rover4 waypoint18 waypoint11)
	(can_traverse rover4 waypoint11 waypoint18)
	(can_traverse rover4 waypoint18 waypoint14)
	(can_traverse rover4 waypoint14 waypoint18)
	(can_traverse rover4 waypoint18 waypoint16)
	(can_traverse rover4 waypoint16 waypoint18)
	(can_traverse rover4 waypoint18 waypoint22)
	(can_traverse rover4 waypoint22 waypoint18)
	(can_traverse rover4 waypoint18 waypoint27)
	(can_traverse rover4 waypoint27 waypoint18)
	(can_traverse rover4 waypoint10 waypoint3)
	(can_traverse rover4 waypoint3 waypoint10)
	(can_traverse rover4 waypoint10 waypoint4)
	(can_traverse rover4 waypoint4 waypoint10)
	(can_traverse rover4 waypoint10 waypoint8)
	(can_traverse rover4 waypoint8 waypoint10)
	(can_traverse rover4 waypoint10 waypoint9)
	(can_traverse rover4 waypoint9 waypoint10)
	(can_traverse rover4 waypoint10 waypoint19)
	(can_traverse rover4 waypoint19 waypoint10)
	(can_traverse rover4 waypoint10 waypoint20)
	(can_traverse rover4 waypoint20 waypoint10)
	(can_traverse rover4 waypoint10 waypoint21)
	(can_traverse rover4 waypoint21 waypoint10)
	(can_traverse rover4 waypoint10 waypoint23)
	(can_traverse rover4 waypoint23 waypoint10)
	(can_traverse rover4 waypoint10 waypoint24)
	(can_traverse rover4 waypoint24 waypoint10)
	(can_traverse rover4 waypoint10 waypoint26)
	(can_traverse rover4 waypoint26 waypoint10)
	(can_traverse rover4 waypoint10 waypoint29)
	(can_traverse rover4 waypoint29 waypoint10)
	(can_traverse rover4 waypoint11 waypoint0)
	(can_traverse rover4 waypoint0 waypoint11)
	(can_traverse rover4 waypoint11 waypoint5)
	(can_traverse rover4 waypoint5 waypoint11)
	(can_traverse rover4 waypoint11 waypoint6)
	(can_traverse rover4 waypoint6 waypoint11)
	(can_traverse rover4 waypoint14 waypoint7)
	(can_traverse rover4 waypoint7 waypoint14)
	(can_traverse rover4 waypoint14 waypoint25)
	(can_traverse rover4 waypoint25 waypoint14)
	(can_traverse rover4 waypoint16 waypoint2)
	(can_traverse rover4 waypoint2 waypoint16)
	(can_traverse rover4 waypoint22 waypoint15)
	(can_traverse rover4 waypoint15 waypoint22)
	(can_traverse rover4 waypoint27 waypoint28)
	(can_traverse rover4 waypoint28 waypoint27)
	(can_traverse rover4 waypoint3 waypoint13)
	(can_traverse rover4 waypoint13 waypoint3)
	(can_traverse rover4 waypoint4 waypoint1)
	(can_traverse rover4 waypoint1 waypoint4)
	(can_traverse rover4 waypoint4 waypoint12)
	(can_traverse rover4 waypoint12 waypoint4)
	(can_traverse rover4 waypoint23 waypoint17)
	(can_traverse rover4 waypoint17 waypoint23)
	(at rover5 waypoint27)
	(available rover5)
	(store_of rover5store rover5)
	(empty rover5store)
	(equipped_for_imaging rover5)
	(can_traverse rover5 waypoint27 waypoint3)
	(can_traverse rover5 waypoint3 waypoint27)
	(can_traverse rover5 waypoint27 waypoint8)
	(can_traverse rover5 waypoint8 waypoint27)
	(can_traverse rover5 waypoint27 waypoint15)
	(can_traverse rover5 waypoint15 waypoint27)
	(can_traverse rover5 waypoint27 waypoint18)
	(can_traverse rover5 waypoint18 waypoint27)
	(can_traverse rover5 waypoint27 waypoint22)
	(can_traverse rover5 waypoint22 waypoint27)
	(can_traverse rover5 waypoint27 waypoint28)
	(can_traverse rover5 waypoint28 waypoint27)
	(can_traverse rover5 waypoint3 waypoint21)
	(can_traverse rover5 waypoint21 waypoint3)
	(can_traverse rover5 waypoint8 waypoint0)
	(can_traverse rover5 waypoint0 waypoint8)
	(can_traverse rover5 waypoint8 waypoint10)
	(can_traverse rover5 waypoint10 waypoint8)
	(can_traverse rover5 waypoint8 waypoint14)
	(can_traverse rover5 waypoint14 waypoint8)
	(can_traverse rover5 waypoint8 waypoint24)
	(can_traverse rover5 waypoint24 waypoint8)
	(can_traverse rover5 waypoint8 waypoint25)
	(can_traverse rover5 waypoint25 waypoint8)
	(can_traverse rover5 waypoint15 waypoint1)
	(can_traverse rover5 waypoint1 waypoint15)
	(can_traverse rover5 waypoint15 waypoint19)
	(can_traverse rover5 waypoint19 waypoint15)
	(can_traverse rover5 waypoint15 waypoint20)
	(can_traverse rover5 waypoint20 waypoint15)
	(can_traverse rover5 waypoint15 waypoint23)
	(can_traverse rover5 waypoint23 waypoint15)
	(can_traverse rover5 waypoint18 waypoint11)
	(can_traverse rover5 waypoint11 waypoint18)
	(can_traverse rover5 waypoint22 waypoint6)
	(can_traverse rover5 waypoint6 waypoint22)
	(can_traverse rover5 waypoint22 waypoint26)
	(can_traverse rover5 waypoint26 waypoint22)
	(can_traverse rover5 waypoint28 waypoint17)
	(can_traverse rover5 waypoint17 waypoint28)
	(can_traverse rover5 waypoint28 waypoint29)
	(can_traverse rover5 waypoint29 waypoint28)
	(can_traverse rover5 waypoint21 waypoint2)
	(can_traverse rover5 waypoint2 waypoint21)
	(can_traverse rover5 waypoint21 waypoint13)
	(can_traverse rover5 waypoint13 waypoint21)
	(can_traverse rover5 waypoint10 waypoint9)
	(can_traverse rover5 waypoint9 waypoint10)
	(can_traverse rover5 waypoint14 waypoint7)
	(can_traverse rover5 waypoint7 waypoint14)
	(can_traverse rover5 waypoint14 waypoint16)
	(can_traverse rover5 waypoint16 waypoint14)
	(can_traverse rover5 waypoint1 waypoint5)
	(can_traverse rover5 waypoint5 waypoint1)
	(can_traverse rover5 waypoint1 waypoint12)
	(can_traverse rover5 waypoint12 waypoint1)
	(can_traverse rover5 waypoint6 waypoint4)
	(can_traverse rover5 waypoint4 waypoint6)
	(at rover6 waypoint3)
	(available rover6)
	(store_of rover6store rover6)
	(empty rover6store)
	(equipped_for_soil_analysis rover6)
	(equipped_for_rock_analysis rover6)
	(can_traverse rover6 waypoint3 waypoint10)
	(can_traverse rover6 waypoint10 waypoint3)
	(can_traverse rover6 waypoint3 waypoint13)
	(can_traverse rover6 waypoint13 waypoint3)
	(can_traverse rover6 waypoint3 waypoint21)
	(can_traverse rover6 waypoint21 waypoint3)
	(can_traverse rover6 waypoint10 waypoint4)
	(can_traverse rover6 waypoint4 waypoint10)
	(can_traverse rover6 waypoint10 waypoint8)
	(can_traverse rover6 waypoint8 waypoint10)
	(can_traverse rover6 waypoint10 waypoint14)
	(can_traverse rover6 waypoint14 waypoint10)
	(can_traverse rover6 waypoint10 waypoint15)
	(can_traverse rover6 waypoint15 waypoint10)
	(can_traverse rover6 waypoint10 waypoint17)
	(can_traverse rover6 waypoint17 waypoint10)
	(can_traverse rover6 waypoint10 waypoint18)
	(can_traverse rover6 waypoint18 waypoint10)
	(can_traverse rover6 waypoint10 waypoint19)
	(can_traverse rover6 waypoint19 waypoint10)
	(can_traverse rover6 waypoint10 waypoint20)
	(can_traverse rover6 waypoint20 waypoint10)
	(can_traverse rover6 waypoint10 waypoint24)
	(can_traverse rover6 waypoint24 waypoint10)
	(can_traverse rover6 waypoint10 waypoint29)
	(can_traverse rover6 waypoint29 waypoint10)
	(can_traverse rover6 waypoint13 waypoint5)
	(can_traverse rover6 waypoint5 waypoint13)
	(can_traverse rover6 waypoint21 waypoint2)
	(can_traverse rover6 waypoint2 waypoint21)
	(can_traverse rover6 waypoint21 waypoint7)
	(can_traverse rover6 waypoint7 waypoint21)
	(can_traverse rover6 waypoint21 waypoint22)
	(can_traverse rover6 waypoint22 waypoint21)
	(can_traverse rover6 waypoint21 waypoint23)
	(can_traverse rover6 waypoint23 waypoint21)
	(can_traverse rover6 waypoint21 waypoint26)
	(can_traverse rover6 waypoint26 waypoint21)
	(can_traverse rover6 waypoint4 waypoint6)
	(can_traverse rover6 waypoint6 waypoint4)
	(can_traverse rover6 waypoint4 waypoint9)
	(can_traverse rover6 waypoint9 waypoint4)
	(can_traverse rover6 waypoint4 waypoint12)
	(can_traverse rover6 waypoint12 waypoint4)
	(can_traverse rover6 waypoint8 waypoint0)
	(can_traverse rover6 waypoint0 waypoint8)
	(can_traverse rover6 waypoint8 waypoint25)
	(can_traverse rover6 waypoint25 waypoint8)
	(can_traverse rover6 waypoint8 waypoint27)
	(can_traverse rover6 waypoint27 waypoint8)
	(can_traverse rover6 waypoint15 waypoint1)
	(can_traverse rover6 waypoint1 waypoint15)
	(can_traverse rover6 waypoint15 waypoint28)
	(can_traverse rover6 waypoint28 waypoint15)
	(can_traverse rover6 waypoint18 waypoint16)
	(can_traverse rover6 waypoint16 waypoint18)
	(can_traverse rover6 waypoint20 waypoint11)
	(can_traverse rover6 waypoint11 waypoint20)
	(at rover7 waypoint29)
	(available rover7)
	(store_of rover7store rover7)
	(empty rover7store)
	(equipped_for_soil_analysis rover7)
	(equipped_for_rock_analysis rover7)
	(equipped_for_imaging rover7)
	(can_traverse rover7 waypoint29 waypoint0)
	(can_traverse rover7 waypoint0 waypoint29)
	(can_traverse rover7 waypoint29 waypoint7)
	(can_traverse rover7 waypoint7 waypoint29)
	(can_traverse rover7 waypoint29 waypoint10)
	(can_traverse rover7 waypoint10 waypoint29)
	(can_traverse rover7 waypoint29 waypoint19)
	(can_traverse rover7 waypoint19 waypoint29)
	(can_traverse rover7 waypoint29 waypoint22)
	(can_traverse rover7 waypoint22 waypoint29)
	(can_traverse rover7 waypoint29 waypoint28)
	(can_traverse rover7 waypoint28 waypoint29)
	(can_traverse rover7 waypoint0 waypoint1)
	(can_traverse rover7 waypoint1 waypoint0)
	(can_traverse rover7 waypoint0 waypoint8)
	(can_traverse rover7 waypoint8 waypoint0)
	(can_traverse rover7 waypoint7 waypoint5)
	(can_traverse rover7 waypoint5 waypoint7)
	(can_traverse rover7 waypoint7 waypoint6)
	(can_traverse rover7 waypoint6 waypoint7)
	(can_traverse rover7 waypoint7 waypoint14)
	(can_traverse rover7 waypoint14 waypoint7)
	(can_traverse rover7 waypoint7 waypoint20)
	(can_traverse rover7 waypoint20 waypoint7)
	(can_traverse rover7 waypoint7 waypoint21)
	(can_traverse rover7 waypoint21 waypoint7)
	(can_traverse rover7 waypoint7 waypoint23)
	(can_traverse rover7 waypoint23 waypoint7)
	(can_traverse rover7 waypoint10 waypoint3)
	(can_traverse rover7 waypoint3 waypoint10)
	(can_traverse rover7 waypoint10 waypoint9)
	(can_traverse rover7 waypoint9 waypoint10)
	(can_traverse rover7 waypoint10 waypoint15)
	(can_traverse rover7 waypoint15 waypoint10)
	(can_traverse rover7 waypoint10 waypoint17)
	(can_traverse rover7 waypoint17 waypoint10)
	(can_traverse rover7 waypoint10 waypoint18)
	(can_traverse rover7 waypoint18 waypoint10)
	(can_traverse rover7 waypoint10 waypoint24)
	(can_traverse rover7 waypoint24 waypoint10)
	(can_traverse rover7 waypoint10 waypoint26)
	(can_traverse rover7 waypoint26 waypoint10)
	(can_traverse rover7 waypoint22 waypoint27)
	(can_traverse rover7 waypoint27 waypoint22)
	(can_traverse rover7 waypoint28 waypoint25)
	(can_traverse rover7 waypoint25 waypoint28)
	(can_traverse rover7 waypoint1 waypoint12)
	(can_traverse rover7 waypoint12 waypoint1)
	(can_traverse rover7 waypoint5 waypoint11)
	(can_traverse rover7 waypoint11 waypoint5)
	(can_traverse rover7 waypoint6 waypoint4)
	(can_traverse rover7 waypoint4 waypoint6)
	(can_traverse rover7 waypoint14 waypoint16)
	(can_traverse rover7 waypoint16 waypoint14)
	(can_traverse rover7 waypoint20 waypoint13)
	(can_traverse rover7 waypoint13 waypoint20)
	(can_traverse rover7 waypoint21 waypoint2)
	(can_traverse rover7 waypoint2 waypoint21)
	(at rover8 waypoint3)
	(available rover8)
	(store_of rover8store rover8)
	(empty rover8store)
	(equipped_for_soil_analysis rover8)
	(equipped_for_rock_analysis rover8)
	(can_traverse rover8 waypoint3 waypoint10)
	(can_traverse rover8 waypoint10 waypoint3)
	(can_traverse rover8 waypoint3 waypoint21)
	(can_traverse rover8 waypoint21 waypoint3)
	(can_traverse rover8 waypoint3 waypoint26)
	(can_traverse rover8 waypoint26 waypoint3)
	(can_traverse rover8 waypoint3 waypoint27)
	(can_traverse rover8 waypoint27 waypoint3)
	(can_traverse rover8 waypoint10 waypoint4)
	(can_traverse rover8 waypoint4 waypoint10)
	(can_traverse rover8 waypoint10 waypoint8)
	(can_traverse rover8 waypoint8 waypoint10)
	(can_traverse rover8 waypoint10 waypoint9)
	(can_traverse rover8 waypoint9 waypoint10)
	(can_traverse rover8 waypoint10 waypoint14)
	(can_traverse rover8 waypoint14 waypoint10)
	(can_traverse rover8 waypoint10 waypoint15)
	(can_traverse rover8 waypoint15 waypoint10)
	(can_traverse rover8 waypoint10 waypoint17)
	(can_traverse rover8 waypoint17 waypoint10)
	(can_traverse rover8 waypoint10 waypoint18)
	(can_traverse rover8 waypoint18 waypoint10)
	(can_traverse rover8 waypoint10 waypoint19)
	(can_traverse rover8 waypoint19 waypoint10)
	(can_traverse rover8 waypoint10 waypoint23)
	(can_traverse rover8 waypoint23 waypoint10)
	(can_traverse rover8 waypoint10 waypoint24)
	(can_traverse rover8 waypoint24 waypoint10)
	(can_traverse rover8 waypoint10 waypoint29)
	(can_traverse rover8 waypoint29 waypoint10)
	(can_traverse rover8 waypoint21 waypoint2)
	(can_traverse rover8 waypoint2 waypoint21)
	(can_traverse rover8 waypoint21 waypoint7)
	(can_traverse rover8 waypoint7 waypoint21)
	(can_traverse rover8 waypoint21 waypoint13)
	(can_traverse rover8 waypoint13 waypoint21)
	(can_traverse rover8 waypoint21 waypoint22)
	(can_traverse rover8 waypoint22 waypoint21)
	(can_traverse rover8 waypoint27 waypoint28)
	(can_traverse rover8 waypoint28 waypoint27)
	(can_traverse rover8 waypoint4 waypoint1)
	(can_traverse rover8 waypoint1 waypoint4)
	(can_traverse rover8 waypoint4 waypoint6)
	(can_traverse rover8 waypoint6 waypoint4)
	(can_traverse rover8 waypoint8 waypoint0)
	(can_traverse rover8 waypoint0 waypoint8)
	(can_traverse rover8 waypoint8 waypoint20)
	(can_traverse rover8 waypoint20 waypoint8)
	(can_traverse rover8 waypoint9 waypoint12)
	(can_traverse rover8 waypoint12 waypoint9)
	(can_traverse rover8 waypoint9 waypoint16)
	(can_traverse rover8 waypoint16 waypoint9)
	(can_traverse rover8 waypoint14 waypoint25)
	(can_traverse rover8 waypoint25 waypoint14)
	(can_traverse rover8 waypoint18 waypoint11)
	(can_traverse rover8 waypoint11 waypoint18)
	(can_traverse rover8 waypoint19 waypoint5)
	(can_traverse rover8 waypoint5 waypoint19)
	(at rover9 waypoint9)
	(available rover9)
	(store_of rover9store rover9)
	(empty rover9store)
	(equipped_for_soil_analysis rover9)
	(equipped_for_imaging rover9)
	(can_traverse rover9 waypoint9 waypoint2)
	(can_traverse rover9 waypoint2 waypoint9)
	(can_traverse rover9 waypoint9 waypoint4)
	(can_traverse rover9 waypoint4 waypoint9)
	(can_traverse rover9 waypoint9 waypoint10)
	(can_traverse rover9 waypoint10 waypoint9)
	(can_traverse rover9 waypoint9 waypoint12)
	(can_traverse rover9 waypoint12 waypoint9)
	(can_traverse rover9 waypoint9 waypoint24)
	(can_traverse rover9 waypoint24 waypoint9)
	(can_traverse rover9 waypoint9 waypoint26)
	(can_traverse rover9 waypoint26 waypoint9)
	(can_traverse rover9 waypoint2 waypoint8)
	(can_traverse rover9 waypoint8 waypoint2)
	(can_traverse rover9 waypoint2 waypoint16)
	(can_traverse rover9 waypoint16 waypoint2)
	(can_traverse rover9 waypoint2 waypoint21)
	(can_traverse rover9 waypoint21 waypoint2)
	(can_traverse rover9 waypoint2 waypoint25)
	(can_traverse rover9 waypoint25 waypoint2)
	(can_traverse rover9 waypoint4 waypoint6)
	(can_traverse rover9 waypoint6 waypoint4)
	(can_traverse rover9 waypoint10 waypoint14)
	(can_traverse rover9 waypoint14 waypoint10)
	(can_traverse rover9 waypoint10 waypoint15)
	(can_traverse rover9 waypoint15 waypoint10)
	(can_traverse rover9 waypoint10 waypoint17)
	(can_traverse rover9 waypoint17 waypoint10)
	(can_traverse rover9 waypoint10 waypoint20)
	(can_traverse rover9 waypoint20 waypoint10)
	(can_traverse rover9 waypoint10 waypoint29)
	(can_traverse rover9 waypoint29 waypoint10)
	(can_traverse rover9 waypoint12 waypoint1)
	(can_traverse rover9 waypoint1 waypoint12)
	(can_traverse rover9 waypoint12 waypoint19)
	(can_traverse rover9 waypoint19 waypoint12)
	(can_traverse rover9 waypoint24 waypoint22)
	(can_traverse rover9 waypoint22 waypoint24)
	(can_traverse rover9 waypoint24 waypoint27)
	(can_traverse rover9 waypoint27 waypoint24)
	(can_traverse rover9 waypoint24 waypoint28)
	(can_traverse rover9 waypoint28 waypoint24)
	(can_traverse rover9 waypoint8 waypoint0)
	(can_traverse rover9 waypoint0 waypoint8)
	(can_traverse rover9 waypoint16 waypoint11)
	(can_traverse rover9 waypoint11 waypoint16)
	(can_traverse rover9 waypoint21 waypoint3)
	(can_traverse rover9 waypoint3 waypoint21)
	(can_traverse rover9 waypoint21 waypoint13)
	(can_traverse rover9 waypoint13 waypoint21)
	(can_traverse rover9 waypoint21 waypoint23)
	(can_traverse rover9 waypoint23 waypoint21)
	(can_traverse rover9 waypoint6 waypoint5)
	(can_traverse rover9 waypoint5 waypoint6)
	(can_traverse rover9 waypoint14 waypoint7)
	(can_traverse rover9 waypoint7 waypoint14)
	(can_traverse rover9 waypoint14 waypoint18)
	(can_traverse rover9 waypoint18 waypoint14)
	(on_board camera0 rover5)
	(calibration_target camera0 objective3)
	(supports camera0 high_res)
	(supports camera0 low_res)
	(on_board camera1 rover3)
	(calibration_target camera1 objective3)
	(supports camera1 high_res)
	(on_board camera2 rover4)
	(calibration_target camera2 objective0)
	(supports camera2 colour)
	(supports camera2 high_res)
	(supports camera2 low_res)
	(on_board camera3 rover7)
	(calibration_target camera3 objective0)
	(supports camera3 colour)
	(supports camera3 high_res)
	(on_board camera4 rover0)
	(calibration_target camera4 objective4)
	(supports camera4 high_res)
	(on_board camera5 rover9)
	(calibration_target camera5 objective4)
	(supports camera5 high_res)
	(supports camera5 low_res)
	(on_board camera6 rover1)
	(calibration_target camera6 objective3)
	(supports camera6 high_res)
	(supports camera6 low_res)
	(visible_from objective0 waypoint0)
	(visible_from objective0 waypoint1)
	(visible_from objective0 waypoint2)
	(visible_from objective0 waypoint3)
	(visible_from objective0 waypoint4)
	(visible_from objective0 waypoint5)
	(visible_from objective0 waypoint6)
	(visible_from objective0 waypoint7)
	(visible_from objective0 waypoint8)
	(visible_from objective0 waypoint9)
	(visible_from objective0 waypoint10)
	(visible_from objective0 waypoint11)
	(visible_from objective0 waypoint12)
	(visible_from objective0 waypoint13)
	(visible_from objective0 waypoint14)
	(visible_from objective0 waypoint15)
	(visible_from objective0 waypoint16)
	(visible_from objective1 waypoint0)
	(visible_from objective1 waypoint1)
	(visible_from objective1 waypoint2)
	(visible_from objective1 waypoint3)
	(visible_from objective1 waypoint4)
	(visible_from objective1 waypoint5)
	(visible_from objective1 waypoint6)
	(visible_from objective1 waypoint7)
	(visible_from objective1 waypoint8)
	(visible_from objective1 waypoint9)
	(visible_from objective1 waypoint10)
	(visible_from objective1 waypoint11)
	(visible_from objective1 waypoint12)
	(visible_from objective1 waypoint13)
	(visible_from objective1 waypoint14)
	(visible_from objective1 waypoint15)
	(visible_from objective1 waypoint16)
	(visible_from objective1 waypoint17)
	(visible_from objective1 waypoint18)
	(visible_from objective1 waypoint19)
	(visible_from objective1 waypoint20)
	(visible_from objective1 waypoint21)
	(visible_from objective1 waypoint22)
	(visible_from objective1 waypoint23)
	(visible_from objective1 waypoint24)
	(visible_from objective1 waypoint25)
	(visible_from objective1 waypoint26)
	(visible_from objective2 waypoint0)
	(visible_from objective2 waypoint1)
	(visible_from objective2 waypoint2)
	(visible_from objective2 waypoint3)
	(visible_from objective2 waypoint4)
	(visible_from objective2 waypoint5)
	(visible_from objective2 waypoint6)
	(visible_from objective3 waypoint0)
	(visible_from objective3 waypoint1)
	(visible_from objective3 waypoint2)
	(visible_from objective3 waypoint3)
	(visible_from objective4 waypoint0)
	(visible_from objective4 waypoint1)
	(visible_from objective4 waypoint2)
	(visible_from objective4 waypoint3)
	(visible_from objective4 waypoint4)
	(visible_from objective5 waypoint0)
	(visible_from objective5 waypoint1)
	(visible_from objective5 waypoint2)
	(visible_from objective5 waypoint3)
	(visible_from objective5 waypoint4)
	(visible_from objective5 waypoint5)
	(visible_from objective5 waypoint6)
	(visible_from objective5 waypoint7)
	(visible_from objective5 waypoint8)
	(visible_from objective5 waypoint9)
	(visible_from objective5 waypoint10)
	(visible_from objective5 waypoint11)
	(visible_from objective5 waypoint12)
	(visible_from objective5 waypoint13)
	(visible_from objective5 waypoint14)
	(visible_from objective5 waypoint15)
	(visible_from objective5 waypoint16)
	(visible_from objective5 waypoint17)
	(visible_from objective5 waypoint18)
	(visible_from objective5 waypoint19)
	(visible_from objective5 waypoint20)
	(visible_from objective5 waypoint21)
	(visible_from objective5 waypoint22)
	(visible_from objective5 waypoint23)
	(visible_from objective5 waypoint24)
	(visible_from objective5 waypoint25)
	(visible_from objective5 waypoint26)
	(visible_from objective5 waypoint27)
	(visible_from objective5 waypoint28)
	(visible_from objective5 waypoint29)
)

(:goal (and
(communicated_soil_data waypoint24)
(communicated_soil_data waypoint2)
(communicated_soil_data waypoint1)
(communicated_soil_data waypoint12)
(communicated_soil_data waypoint11)
(communicated_soil_data waypoint21)
(communicated_soil_data waypoint3)
(communicated_soil_data waypoint25)
(communicated_rock_data waypoint6)
(communicated_rock_data waypoint28)
(communicated_rock_data waypoint20)
(communicated_image_data objective1 low_res)
(communicated_image_data objective5 high_res)
(communicated_image_data objective0 high_res)
	)
)


(:constraints (and (sometime-before (or (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint24 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint2 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint1 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint12 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint11 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint21 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint3 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint25 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_rock_data ?r general waypoint6 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_rock_data ?r general waypoint28 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_rock_data ?r general waypoint20 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_image_data ?r general objective1 low_res ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_image_data ?r general objective5 high_res ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_image_data ?r general objective0 high_res ?x waypoint27))) (exists (?r - rover ?s - store) (sample_soil ?r ?s waypoint24)))
(sometime-before (or (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint24 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint2 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint1 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint12 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint11 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint21 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint3 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint25 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_rock_data ?r general waypoint6 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_rock_data ?r general waypoint28 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_rock_data ?r general waypoint20 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_image_data ?r general objective1 low_res ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_image_data ?r general objective5 high_res ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_image_data ?r general objective0 high_res ?x waypoint27))) (exists (?r - rover ?s - store) (sample_soil ?r ?s waypoint2)))
(sometime-before (or (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint24 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint2 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint1 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint12 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint11 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint21 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint3 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint25 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_rock_data ?r general waypoint6 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_rock_data ?r general waypoint28 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_rock_data ?r general waypoint20 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_image_data ?r general objective1 low_res ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_image_data ?r general objective5 high_res ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_image_data ?r general objective0 high_res ?x waypoint27))) (exists (?r - rover ?s - store) (sample_soil ?r ?s waypoint1)))
(sometime-before (or (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint24 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint2 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint1 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint12 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint11 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint21 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint3 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint25 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_rock_data ?r general waypoint6 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_rock_data ?r general waypoint28 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_rock_data ?r general waypoint20 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_image_data ?r general objective1 low_res ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_image_data ?r general objective5 high_res ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_image_data ?r general objective0 high_res ?x waypoint27))) (exists (?r - rover ?s - store) (sample_soil ?r ?s waypoint12)))
(sometime-before (or (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint24 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint2 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint1 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint12 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint11 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint21 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint3 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint25 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_rock_data ?r general waypoint6 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_rock_data ?r general waypoint28 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_rock_data ?r general waypoint20 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_image_data ?r general objective1 low_res ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_image_data ?r general objective5 high_res ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_image_data ?r general objective0 high_res ?x waypoint27))) (exists (?r - rover ?s - store) (sample_soil ?r ?s waypoint11)))
(sometime-before (or (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint24 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint2 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint1 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint12 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint11 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint21 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint3 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint25 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_rock_data ?r general waypoint6 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_rock_data ?r general waypoint28 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_rock_data ?r general waypoint20 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_image_data ?r general objective1 low_res ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_image_data ?r general objective5 high_res ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_image_data ?r general objective0 high_res ?x waypoint27))) (exists (?r - rover ?s - store) (sample_soil ?r ?s waypoint21)))
(sometime-before (or (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint24 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint2 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint1 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint12 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint11 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint21 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint3 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint25 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_rock_data ?r general waypoint6 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_rock_data ?r general waypoint28 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_rock_data ?r general waypoint20 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_image_data ?r general objective1 low_res ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_image_data ?r general objective5 high_res ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_image_data ?r general objective0 high_res ?x waypoint27))) (exists (?r - rover ?s - store) (sample_soil ?r ?s waypoint3)))
(sometime-before (or (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint24 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint2 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint1 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint12 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint11 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint21 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint3 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint25 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_rock_data ?r general waypoint6 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_rock_data ?r general waypoint28 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_rock_data ?r general waypoint20 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_image_data ?r general objective1 low_res ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_image_data ?r general objective5 high_res ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_image_data ?r general objective0 high_res ?x waypoint27))) (exists (?r - rover ?s - store) (sample_soil ?r ?s waypoint25)))
(sometime-before (or (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint24 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint2 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint1 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint12 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint11 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint21 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint3 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint25 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_rock_data ?r general waypoint6 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_rock_data ?r general waypoint28 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_rock_data ?r general waypoint20 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_image_data ?r general objective1 low_res ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_image_data ?r general objective5 high_res ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_image_data ?r general objective0 high_res ?x waypoint27))) (exists (?r - rover ?s - store) (sample_rock ?r ?s waypoint6)))
(sometime-before (or (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint24 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint2 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint1 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint12 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint11 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint21 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint3 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint25 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_rock_data ?r general waypoint6 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_rock_data ?r general waypoint28 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_rock_data ?r general waypoint20 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_image_data ?r general objective1 low_res ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_image_data ?r general objective5 high_res ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_image_data ?r general objective0 high_res ?x waypoint27))) (exists (?r - rover ?s - store) (sample_rock ?r ?s waypoint28)))
(sometime-before (or (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint24 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint2 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint1 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint12 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint11 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint21 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint3 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint25 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_rock_data ?r general waypoint6 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_rock_data ?r general waypoint28 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_rock_data ?r general waypoint20 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_image_data ?r general objective1 low_res ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_image_data ?r general objective5 high_res ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_image_data ?r general objective0 high_res ?x waypoint27))) (exists (?r - rover ?s - store) (sample_rock ?r ?s waypoint20)))
(sometime-before (or (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint24 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint2 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint1 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint12 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint11 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint21 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint3 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint25 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_rock_data ?r general waypoint6 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_rock_data ?r general waypoint28 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_rock_data ?r general waypoint20 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_image_data ?r general objective1 low_res ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_image_data ?r general objective5 high_res ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_image_data ?r general objective0 high_res ?x waypoint27))) (exists (?r - rover ?p - waypoint ?i - camera) (take_image ?r ?p objective1 ?i low_res)))
(sometime-before (or (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint24 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint2 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint1 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint12 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint11 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint21 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint3 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint25 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_rock_data ?r general waypoint6 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_rock_data ?r general waypoint28 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_rock_data ?r general waypoint20 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_image_data ?r general objective1 low_res ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_image_data ?r general objective5 high_res ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_image_data ?r general objective0 high_res ?x waypoint27))) (exists (?r - rover ?p - waypoint ?i - camera) (take_image ?r ?p objective5 ?i high_res)))
(sometime-before (or (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint24 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint2 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint1 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint12 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint11 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint21 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint3 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint25 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_rock_data ?r general waypoint6 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_rock_data ?r general waypoint28 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_rock_data ?r general waypoint20 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_image_data ?r general objective1 low_res ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_image_data ?r general objective5 high_res ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_image_data ?r general objective0 high_res ?x waypoint27))) (exists (?r - rover ?p - waypoint ?i - camera) (take_image ?r ?p objective0 ?i high_res)))
(sometime-before (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint24 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint2 ?x waypoint27)))
(sometime-before (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint2 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint1 ?x waypoint27)))
(sometime-before (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint1 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint12 ?x waypoint27)))
(sometime-before (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint12 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint11 ?x waypoint27)))
(sometime-before (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint11 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint21 ?x waypoint27)))
(sometime-before (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint21 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint3 ?x waypoint27)))
(sometime-before (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint3 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint25 ?x waypoint27)))
(sometime-before (exists (?r - rover ?x - waypoint) (communicate_soil_data ?r general waypoint25 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_rock_data ?r general waypoint6 ?x waypoint27)))
(sometime-before (exists (?r - rover ?x - waypoint) (communicate_rock_data ?r general waypoint6 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_rock_data ?r general waypoint28 ?x waypoint27)))
(sometime-before (exists (?r - rover ?x - waypoint) (communicate_rock_data ?r general waypoint28 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_rock_data ?r general waypoint20 ?x waypoint27)))
(sometime-before (exists (?r - rover ?x - waypoint) (communicate_rock_data ?r general waypoint20 ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_image_data ?r general objective1 low_res ?x waypoint27)))
(sometime-before (exists (?r - rover ?x - waypoint) (communicate_image_data ?r general objective1 low_res ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_image_data ?r general objective5 high_res ?x waypoint27)))
(sometime-before (exists (?r - rover ?x - waypoint) (communicate_image_data ?r general objective5 high_res ?x waypoint27)) (exists (?r - rover ?x - waypoint) (communicate_image_data ?r general objective0 high_res ?x waypoint27)))
(always (and (forall (?r - rover ?x - waypoint) (not (communicate_soil_data ?r general waypoint0 ?x waypoint27)))
(forall (?r - rover ?s - store) (not (sample_soil ?r ?s waypoint0)))
(forall (?r - rover ?x - waypoint) (not (communicate_image_data ?r general objective2 low_res ?x waypoint27)))
(forall (?r - rover ?p - waypoint ?i - camera) (not (take_image ?r ?p objective2 ?i colour)))
(forall (?r - rover ?s - store) (not (sample_rock ?r ?s waypoint25)))
(forall (?r - rover ?x - waypoint) (not (communicate_image_data ?r general objective4 high_res ?x waypoint27)))
(forall (?r - rover ?x - waypoint) (not (communicate_rock_data ?r general waypoint4 ?x waypoint27)))
(forall (?r - rover ?x - waypoint) (not (communicate_rock_data ?r general waypoint29 ?x waypoint27)))
(forall (?r - rover ?p - waypoint ?i - camera) (not (take_image ?r ?p objective4 ?i low_res)))
(forall (?r - rover ?x - waypoint) (not (communicate_soil_data ?r general waypoint23 ?x waypoint27)))
(forall (?r - rover ?x - waypoint) (not (communicate_rock_data ?r general waypoint14 ?x waypoint27)))
(forall (?r - rover ?p - waypoint ?i - camera) (not (take_image ?r ?p objective2 ?i high_res)))
(forall (?r - rover ?x - waypoint) (not (communicate_image_data ?r general objective2 colour ?x waypoint27)))
(forall (?r - rover ?p - waypoint ?i - camera) (not (take_image ?r ?p objective4 ?i high_res)))
(forall (?r - rover ?s - store) (not (sample_rock ?r ?s waypoint29)))
(forall (?r - rover ?s - store) (not (sample_rock ?r ?s waypoint24)))
(forall (?r - rover ?x - waypoint) (not (communicate_rock_data ?r general waypoint15 ?x waypoint27)))
(forall (?r - rover ?p - waypoint ?i - camera) (not (take_image ?r ?p objective5 ?i low_res)))
(forall (?r - rover ?s - store) (not (sample_soil ?r ?s waypoint23)))
(forall (?r - rover ?s - store) (not (sample_rock ?r ?s waypoint14)))
(forall (?r - rover ?s - store) (not (sample_rock ?r ?s waypoint12)))
(forall (?r - rover ?p - waypoint ?i - camera) (not (take_image ?r ?p objective1 ?i high_res)))
(forall (?r - rover ?s - store) (not (sample_rock ?r ?s waypoint9)))
(forall (?r - rover ?x - waypoint) (not (communicate_rock_data ?r general waypoint12 ?x waypoint27)))
(forall (?r - rover ?x - waypoint) (not (communicate_image_data ?r general objective3 low_res ?x waypoint27)))
(forall (?r - rover ?s - store) (not (sample_rock ?r ?s waypoint19)))
(forall (?r - rover ?x - waypoint) (not (communicate_image_data ?r general objective3 high_res ?x waypoint27)))
(forall (?r - rover ?x - waypoint) (not (communicate_image_data ?r general objective0 colour ?x waypoint27)))
(forall (?r - rover ?x - waypoint) (not (communicate_image_data ?r general objective0 low_res ?x waypoint27)))
(forall (?r - rover ?s - store) (not (sample_rock ?r ?s waypoint26)))
(forall (?r - rover ?p - waypoint ?i - camera) (not (take_image ?r ?p objective3 ?i low_res)))
(forall (?r - rover ?p - waypoint ?i - camera) (not (take_image ?r ?p objective0 ?i colour)))
(forall (?r - rover ?x - waypoint) (not (communicate_image_data ?r general objective5 low_res ?x waypoint27)))
(forall (?r - rover ?x - waypoint) (not (communicate_rock_data ?r general waypoint19 ?x waypoint27)))
(forall (?r - rover ?p - waypoint ?i - camera) (not (take_image ?r ?p objective5 ?i colour)))
(forall (?r - rover ?x - waypoint) (not (communicate_image_data ?r general objective4 colour ?x waypoint27)))
(forall (?r - rover ?x - waypoint) (not (communicate_rock_data ?r general waypoint10 ?x waypoint27)))
(forall (?r - rover ?p - waypoint ?i - camera) (not (take_image ?r ?p objective2 ?i low_res)))
(forall (?r - rover ?x - waypoint) (not (communicate_image_data ?r general objective4 low_res ?x waypoint27)))
(forall (?r - rover ?p - waypoint ?i - camera) (not (take_image ?r ?p objective3 ?i colour)))
(forall (?r - rover ?s - store) (not (sample_rock ?r ?s waypoint8)))
(forall (?r - rover ?x - waypoint) (not (communicate_rock_data ?r general waypoint26 ?x waypoint27)))
(forall (?r - rover ?p - waypoint ?i - camera) (not (take_image ?r ?p objective1 ?i colour)))
(forall (?r - rover ?x - waypoint) (not (communicate_image_data ?r general objective2 high_res ?x waypoint27)))
(forall (?r - rover ?x - waypoint) (not (communicate_image_data ?r general objective5 colour ?x waypoint27)))
(forall (?r - rover ?p - waypoint ?i - camera) (not (take_image ?r ?p objective3 ?i high_res)))
(forall (?r - rover ?p - waypoint ?i - camera) (not (take_image ?r ?p objective0 ?i low_res)))
(forall (?r - rover ?x - waypoint) (not (communicate_rock_data ?r general waypoint8 ?x waypoint27)))
(forall (?r - rover ?x - waypoint) (not (communicate_rock_data ?r general waypoint24 ?x waypoint27)))
(forall (?r - rover ?s - store) (not (sample_rock ?r ?s waypoint3)))
(forall (?r - rover ?s - store) (not (sample_rock ?r ?s waypoint4)))
(forall (?r - rover ?x - waypoint) (not (communicate_image_data ?r general objective1 colour ?x waypoint27)))
(forall (?r - rover ?x - waypoint) (not (communicate_image_data ?r general objective1 high_res ?x waypoint27)))
(forall (?r - rover ?x - waypoint) (not (communicate_rock_data ?r general waypoint3 ?x waypoint27)))
(forall (?r - rover ?x - waypoint) (not (communicate_image_data ?r general objective3 colour ?x waypoint27)))
(forall (?r - rover ?p - waypoint ?i - camera) (not (take_image ?r ?p objective4 ?i colour)))
(forall (?r - rover ?x - waypoint) (not (communicate_rock_data ?r general waypoint9 ?x waypoint27)))
(forall (?r - rover ?s - store) (not (sample_rock ?r ?s waypoint15)))
(forall (?r - rover ?x - waypoint) (not (communicate_rock_data ?r general waypoint25 ?x waypoint27)))
(forall (?r - rover ?s - store) (not (sample_rock ?r ?s waypoint10)))))))

)